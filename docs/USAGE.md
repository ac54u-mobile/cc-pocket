...
- 一部手机：iPhone（iOS 17+，[TrollStore IPA](https://github.com/ac54u-mobile/cc-pocket/releases?q=app-v)）或 Android（从 [GitHub Releases](https://github.com/ac54u-mobile/cc-pocket/releases) 下载 APK）。
- 应用 ID：`com.txx.ccpocket`；默认 relay：`wss://relay.txx.app`。
...
- 升级：再跑一遍上面的安装命令，或 `cc-pocket-daemon update`；
...
- Codex IDE 与 iOS 打开同一 thread 后可实时双向同步；这需要 App `1.5.3+`、daemon `1.0.3+` 和 installer 管理的 standalone Codex。完整安装、验收与排错见 [iOS 与 Codex IDE 实时双向同步教程](./CODEX_IDE_SYNC.md)。
...
# 卸掉服务后删除 ~/.local/share/cc-pocket（或 Windows %LOCALAPPDATA%\cc-pocket）
```
